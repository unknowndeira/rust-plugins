    ...
    class Clans : CovalencePlugin
    {
        ...
        public class Clan
        {
            ...
            internal bool LeaveClan(IPlayer player)
            {
                ...
                if (basePlayer != null)
                {
                    basePlayer.Team?.RemovePlayer(basePlayer.userID);
            ...
            internal bool KickMember(IPlayer player, string targetId)
            {
                ...
                {
                    member.Player.Reply(Message("Notification.Disband.Reply", member.Player.Id));
                    OnPlayerDisconnected(member.Player);
                    #if RUST
                    BasePlayer basePlayer = BasePlayer.FindByID(ulong.Parse(targetId));
                    if (basePlayer != null)
                    {
                        basePlayer.Team?.RemovePlayer(basePlayer.userID);
                        basePlayer.SendNetworkUpdate();
            ...
            internal void DisbandClan()
            {
                ...
                {
                    ClanMembers.Remove(memberId);
                    Instance.storedData.UnregisterPlayer(memberId);
                    
                    // Notify online members and remove from team UI
                    IPlayer memberPlayer = Instance.covalence.Players.FindPlayerById(memberId);
                    if (memberPlayer != null && memberPlayer.IsConnected)
                    {
                        memberPlayer.Reply(string.Format(Message("Notification.Disband.Reply", memberPlayer.Id)));
                        #if RUST
                        BasePlayer basePlayer = BasePlayer.FindByID(ulong.Parse(memberId));
                        if (basePlayer != null)
                        {
                        basePlayer.Team?.RemovePlayer(basePlayer.userID);
                        basePlayer.SendNetworkUpdate();
